...
import com.financialplanner.service.OpenAIService;
...
public class TransactionController {
    ...
    public ResponseEntity<Map<String, Object>> getAIStatus() {
        ...
        if (available) {
            ...
            status.put("message", "✅ OpenAI is configured and ready! Real AI-powered financial insights active.");
            ...
            status.put("message", "❌ OpenAI is not configured. Add your API key to environment variables");
            status.put("setupGuide", "Get your free API key at: https://platform.openai.com/");
        }
    ...
    public ResponseEntity<Map<String, Object>> testAI(@RequestBody Map<String, String> request) {
        ...
        log.info("Testing OpenAI with prompt: {}", prompt);
        
        boolean available = openAIService.isAvailable();
        if (!available) {
            response.put("success", false);
            response.put("error", "OpenAI is not configured");
            response.put("message", "Please add your OPENAI_API_KEY to environment variables");
            response.put("setupGuide", "Get your API key at: https://platform.openai.com/");
        ...
        try {
            ...
            response.put("message", "🎉 OpenAI is working! This is a REAL AI-generated response from GPT.");
            
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            log.error("Error testing OpenAI: {}", e.getMessage());
            response.put("success", false);
    ...
    public ResponseEntity<Map<String, Object>> getSpendingInsights(
        ...
        try {
            ...
            String aiInsight = openAIService.generateSpendingInsight(
                period, 
                categorySpending, 
            ...
            insights.put("aiPowered", openAIService.isAvailable());
    ...
    public ResponseEntity<Map<String, Object>> predictCategory(@RequestBody Map<String, Object> request) {
        ...
        if (aiAvailable) {
            try {
                predictedCategory = openAIService.categorizeTransaction(
                    merchantName, 
                    description, 
        ...
        response.put("message", predictedCategory != null ? 
            "Category predicted by OpenAI GPT" : 
    ...
    public ResponseEntity<Map<String, Object>> getFinancialTips() {
        ...
        if (!openAIService.isAvailable()) {
            response.put("success", false);
            response.put("message", "OpenAI AI not configured");
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(response);
        }
        ...
        try {
            ...
            String tips = openAIService.generateRecommendations(prompt);
            
            response.put("success", true);
            response.put("tips", tips);
            response.put("provider", "OpenAI GPT");
    ...
    public ResponseEntity<Map<String, String>> healthCheck() {
        ...
        health.put("service", "Financial Planner API with OpenAI GPT");
        health.put("timestamp", LocalDateTime.now().toString());
        health.put("ai_provider", "OpenAI GPT");
        health.put("ai_enabled", String.valueOf(openAIService.isAvailable()));
        health.put("ai_model", "gpt-4o-mini");
